...
class DetRecFunctions(object):
    def __init__(self, use_large=False, det_file=ai_cfg.DET_PATH, small_rec_file=ai_cfg.SMALL_REC_PATH,
                 large_rec_file=ai_cfg.LARGE_REC_PATH, ppocr_keys=ai_cfg.PPOCR_KEYS_PATH):
        self.onet_det_session = onnxruntime.InferenceSession(det_file)
        if use_large:
            self.onet_rec_session = onnxruntime.InferenceSession(large_rec_file)
        else:
            self.onet_rec_session = onnxruntime.InferenceSession(small_rec_file)
    ...
    def filter_tag_det_res(self, dt_boxes, image_shape):
        ...
        dt_boxes_new = []
        for box in dt_boxes:
            box = self.order_points_clockwise(box)   # 检测框后处理
            box = self.clip_det_res(box, img_height, img_width)
            rect_width = int(np.linalg.norm(box[0] - box[1]))
            rect_height = int(np.linalg.norm(box[0] - box[3]))
            if rect_width <= 3 or rect_height <= 3:
                continue
            dt_boxes_new.append(box)  # 得到检测框
    ...
    # 推理检测图片中的部分
    def get_boxes(self, img_ori):
        img_part = img_ori.copy()  # 获取原图像
        data_part = {'image': img_part}
        data_part = self.transform(data_part, self.infer_before_process_op)  # 图像预处理
        img_part, shape_part_list = data_part
        img_part = np.expand_dims(img_part, axis=0)
        shape_part_list = np.expand_dims(shape_part_list, axis=0)   # 调整数据结构
        inputs_part = {self.onet_det_session.get_inputs()[0].name: img_part}
        outs_part = self.onet_det_session.run(None, inputs_part)    # 文本框检测模型预测
        post_res_part = self.det_re_process_op(outs_part[0], shape_part_list)   # 文本框检测结果解析
        dt_boxes_part = post_res_part[0]['points']    # 获取检测位置
        dt_boxes_part = self.filter_tag_det_res(dt_boxes_part, img_ori.shape)  # 检测位置后处理
        dt_boxes_part = self.sorted_boxes(dt_boxes_part)  # 对文本框进行重新排序，按照从左到右，从上到下的顺序
    ...
    def recognition_img(self, img_ori, dt_boxes):
        # img_ori = self.img
        img = img_ori.copy()
        ### 识别过程
        ## 根据bndbox得到小图片
...
def drawTextDetRes(dt_boxes, img):
    src_im = img
    for box in dt_boxes:
