...
from plugin.web.rest_api.posen_value import posen_status_bp,setStatus

# logging.getLogger("werkzeug").setLevel(logging.WARNING)
app = Flask(__name__)
# app.register_blueprint(posen_status_bp)
...
class FlaskTask():
    ...
    def worker(self, host="127.0.0.1", port=8082, q_flask=None, full_dict=None):
        """
        flask可视化交互界面启动插件
        ...
        def camera():
            while True:
                if q_flask.empty():
                    ...
                    img = q_flask.get()
                    if img != False:
                        img = np.array(img).reshape(450, 950, 3)
