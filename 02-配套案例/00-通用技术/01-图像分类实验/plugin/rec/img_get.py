...
from base.fruits_detecte_rec import recImgDis
from tools.config import config

class VideoThread(threading.Thread):
    def __init__(self, camera=0, q_flask:Queue=None,
                 q_img:Queue=None, q_rec:Queue=None, full_dict=None):
        threading.Thread.__init__(self)
        self.cap = setCamera(camera)    # 网络摄像头
        self.q_flask = q_flask          # 消息队列传递 绘制识别结果后的图像到web显示插件
        ...
        while True:
            if self.cap != "":
                ...
                if not self.q_rec.empty():
                    fruits_detecte_pricet = self.q_rec.get()

                frame, det = recImgDis(frame, fruits_detecte_pricet)  # 调用识别函数，识别图像
                self.full_dict[config.FRUIT_LIAB] = det

                # 传递图像到web显示界面中
                if not self.q_flask.full() and not frame is None:
                    self.q_flask.put(bytearray(frame))

                if config.IMG_DIS_FLAGE:  # 判断是否在本地打开窗口显示结果
...
class ImageGet(object):
    ...
    # 执行函数，
    def worker(self, q_flask:Queue=None, q_img:Queue=None, q_rec:Queue=None, full_dict=None):
        self.video_thread = VideoThread(config.USB_CAMERA_INDEX, q_flask, q_img, q_rec, full_dict)
        log.info("图像获取线程已启动！！")
        self.video_thread.start()


# 构建plugin
def imgGetPluginRegist(q_flask:Queue=None, q_img:Queue=None, q_rec:Queue=None, full_dict=None):
