...
class GarbageRec(object):
    ...
    def inference(self, img, auto=False):
        ...
        if auto:
            ...
            if socr > 0.65:
                dat = ai_cfg.GARBAGE_LABELS[np.argmax(predictions)]
                print(dat)

                if dat in ["牛奶盒", "易拉罐", "废纸箱", "塑料瓶"]:  # 可回收垃圾
                    dat_index = 0
                elif dat in ["苹果核", "香蕉皮", "菜叶", "鱼骨头"]:  # 厨余垃圾
                    dat_index = 1
                elif dat in ["药丸", "油漆桶",  "注射器",  "电池"]:  # 有害垃圾
                    dat_index = 2
                elif dat in ["筷子", "抹布", "烟蒂", "大骨头"]:     # 其它垃圾
                    dat_index = 3
                else:
                    dat_index = -1
...
if __name__ == '__main__':
    ...
    comm_rec = GarbageRec("./resource/model_zoo/garbage_model.tflite")
    ...
    while True:
        ...
        frame = putText(frame, ai_cfg.GARBAGE_LABELS[np.argmax(y_pred)], org=(0, 0))
