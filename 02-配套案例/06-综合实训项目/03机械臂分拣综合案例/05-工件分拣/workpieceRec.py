...
FRUITS_MODEL_PATH = "./resource/model_zoo/workpiece.tflite"
MAP_PARAM_PATH = "./tools/CameraCalibration/map_param.npz"

class WorkPieceDetec(object):
    ...
    def inference(self, img, auto=False):
        ...
        if auto:
            ...
            if socr > ai_cfg.DIS_THR:
                dat = ai_cfg.workpiece_labels[np.argmax(predictions)]
                print(dat)
                if dat in ['齿轮', '销钉']:
                    dat_index = 0
                elif dat in ['垫片', '螺钉', '螺柱']:
                    dat_index = 1
                elif dat in ['膨胀螺丝钩', '铆钉', '六角螺栓']:
                    dat_index = 2
                elif dat in ['螺母', '双头螺栓']:
                    dat_index = 3
                else:
