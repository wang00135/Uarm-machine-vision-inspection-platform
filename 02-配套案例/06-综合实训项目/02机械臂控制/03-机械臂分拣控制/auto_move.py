...
class ArmServo(object):
    ...
    def moveUp(self, movetime=1000):
        # 机械臂向上移动
        self.arm.set_polar(rotation=90, stretch=90, height=50,
                           timeout=movetime)
    ...
    def servosMove1(self, coordinate_data, movetime=1500, wait_flag=True):
        # 机械臂移动控制
        self.arm.set_position(x=coordinate_data[0],
                           y=coordinate_data[1],
                           z=coordinate_data[2], timeout=movetime, speed=50)
        if wait_flag:
            time.sleep(2)
    ...
    def autoMove(self, id):
        if id >= 0 and id <= 3:
            ...
            self.servoAngle(self.arm_polar_val[id]['angle'])

    def imgRecCtr(self, X, Y, store_index):
        if X:
            x, y, z = X, Y * 10, self.polar_height
            x = 90 + x * self.x_weight
            # x = X*10
            self.moveUp()  # 机械臂归位
            print("store_index22：", store_index)

            # time.sleep(1)
            self.servosMove((x, y, z+50), movetime=1500, wait_flag=True)
            self.servoAngle(x)  # 旋转角度控制
            time.sleep(1)
            self.servosMove((x, y, z), movetime=1500, wait_flag=True)  # 色块位置
            self.armClampBlock(True)  # 机械臂爪子抓取
            # self.moveUp()  # 归位
            self.servosMove1(store_index, movetime=1500, wait_flag=True)
            self.servoAngle(90)  # 爪子旋转角度控制
...
if __name__ == "__main__":
    arm = ArmServo()  # 机械臂基本控制
    trans_form = TransForm()    # 坐标映射

    img_center = (100, 200)  # 图像中的像素位置
    img_size = (320, 240)    # 图像大小
    store_index = (159.43, 141.8, 5)
    world_x, world_y = trans_form.convertCoordinate(img_center[0], img_center[1], img_size)  # 坐标映射，将图像中的像素位置转换为现实世界坐标

    print("world_x, world_y:",world_x, world_y)
    arm.imgRecCtr(world_x, world_y, store_index)  # 机械臂移动
