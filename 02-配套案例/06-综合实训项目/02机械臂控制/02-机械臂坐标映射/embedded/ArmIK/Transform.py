...
import math
import numpy as np
from embedded.ArmIK import CalibrationConfig


class TransForm(object):
    def __init__(self, map_param_path='./embedded/ArmIK/map_param.npz'):

        # 机械臂原点即云台中心，距离摄像头画面中心的距离， 单位cm
        self.image_center_distance = 14.5
    ...
    def map_parm(self,x,l):
        # 根据色块的真实长度和图像中的像素值做映射，将图像中的像素位置映射到真实距离位置
        return int(x*(CalibrationConfig.square_length/l))

    def convertCoordinate(self, x, y, size):
        # 将图形的像素坐标转换为机械臂的坐标系
        # 传入坐标及图像分辨率，例如(100, 100, (640, 320))
        x = self.leMap(x, 0, size[0], 0, 320)
        x = x - 160
        x_ = round(x * self.map_param_*1.2, 2)

        y = self.leMap(y, 0, size[1], 0, 240)
        y = y - 120
    ...
    def getCoordinate(self, img, img_centerx, img_centery):
        ...
        if areaMaxContour is not None:
            ...
            if area_max > 2500:  # 有找到最大面积
                ...
                rotation_angle = rect[2]  # 返回矩形的旋转角度
...
if __name__ == "__main__":
    ...
    img_center = (200, 200)  # 图像中的像素位置
    img_size = (640, 480)    # 图像像素大小
    square_img_length = (60,60)   # 色块在图像中的x边长和y边长
    world_x, world_y = trans_form.convertCoordinate(img_center[0], img_center[1], img_size, square_img_length)  # 坐标映射，将图像中的像素位置转换为现实世界坐标
