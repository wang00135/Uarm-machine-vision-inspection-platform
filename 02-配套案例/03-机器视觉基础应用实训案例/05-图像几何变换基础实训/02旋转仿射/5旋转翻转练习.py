...
dst1 = cv.warpAffine(img, M, (cols * 2, rows * 2), flags = cv.INTER_LINEAR, borderMode = cv.BORDER_WRAP)

cv.imwrite('./outputs/fig.jpg', dst)
cv.imwrite('./outputs/fig1.jpg', dst1)

cv.imshow('img', np.hstack((img, dst)))
cv.imshow('img_1', dst1)
