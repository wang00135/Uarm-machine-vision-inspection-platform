from ai_lib.components.utils import getFaceBoxs, putText
...
from ai_lib.all_img_rec import AllImgRec
...
class BoxDetectRec(object):
    def __init__(self, box_det_path=BOX_DET_PATH,
        ...
        self.all_img_rec = AllImgRec()

        # self.fruit_rec = FruitDetect(model_path=fruit_rec_path, map_param_path=map_param_path)
    ...
    def inference(self, img, img_rec=False, img_mode=0):
        ...
        for i, box in enumerate(boxs):
            ...
            try:
                ...
                if img_rec:
                    imclass, imgindex = self.all_img_rec.run(img_mode, box_roi, auto=True)
                    box_label.append(imclass)
                    box_index.append(imgindex)

                coord = self.trans_form.getCoordinate(box_roi, *boxs_centre[i])
                if coord[0] != None:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        st = time.time()
        box_pricet = box_det_rec.inference(img, True, img_mode=2)
        img, box_std = recImgDis(img, box_pricet)

        box_det_rec.adjust(img, box_std['boxs'])
        ...
        if box_std["std"] != "none":
            if box_std["box_coordin"]:
                ...
                if count > 30:
                    ...
                    arm_servo.imgRecCtr(box_std["box_coordin"], box_std['box_index'])
            else:
                count = 0
            print(box_std["box_index"])
                # emb_dat.servosMove((box_std["box_coordin"][0][0], box_std["box_coordin"][0][1], 4), -90, -90, 90)
            # x, z = cmp(face_std["face_circle"])               # 将图像坐标映射到机械臂坐标
            # print("x:", x)
            # emb_dat.roobotCmod(int(x), 120, int(z), tim=20)   # 控制机械臂执行动作
